...
export const registerTaskTools: ToolModule = (server, client) => {
  server.registerTool(
    "list_tasks",
    {
      title: "List tasks",
      description:
        "List tasks visible to the current user, filterable by project and date range. " +
        "Completed tasks are excluded unless include_completed is true.",
      inputSchema: schema.listTasks,
      annotations: { readOnlyHint: true },
    },
    async ({ project_ids, ...rest }) =>
      run(() => client.get("/v1/tasks", { ...rest, "project_ids[]": project_ids })),
  );
  ...
  server.registerTool(
    ...
    async ({ task_id, type }) =>
      run(() => client.get(`/v1/tasks/${task_id}/resources`, { type })),
  );
  ...
  server.registerTool(
    "list_groups",
    {
      title: "List groups",
      description:
        "List the task groups of a project. Groups are the containers tasks live in — " +
        "use this to find parent_group_id values for create_task.",
      inputSchema: schema.listGroups,
      annotations: { readOnlyHint: true },
    },
    async ({ project_id, flatten_children }) =>
      run(() =>
        client.get("/v1/groups", { "project_ids[]": [project_id], flatten_children }),
      ),
  );
